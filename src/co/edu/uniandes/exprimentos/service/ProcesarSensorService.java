...
import co.edu.uniandes.exprimentos.main.NodoConfig;
...
public class ProcesarSensorService implements SensorService {
	...
	public ProcesarSensorService(){
		runnableQueue = new ArrayBlockingQueue<Runnable>(10000);
		//ThreadPoolSingleton.getInstance();
		cifrador = new Cifrador(NodoConfig.PRIVATE_KEY, null);
		threadPool = new ThreadPoolExecutor(2000, 5000, 5000, TimeUnit.MILLISECONDS, runnableQueue);
		Reglas.llenarReglas();
	}
	 ...
	 */
	@Override
	public boolean procesar(byte[] tramaCifrada) {
		byte[] trama = cifrador.decrypt(tramaCifrada);
		if(trama != null && trama.length > 0){

			if(comprobarIntegridad(trama))
			{	
				...
				try{
					...
					for (int i = 10; i <= 20 ; i++) {
						...
						for(int j = 1, k = 1; j <= 4; j++, k*=2){
							...
							EventThread hilo = new EventThread();//ThreadPoolSingleton.getInstance().getThread();
	...
	/**
	 * Metodo que aplica el metodo de verficacion del algoritmo CRC16 para verificar 
	 * la integridad de la trama
	 * @param tramaCifrada trama enviada por el cliente para ser verificada
	 * @return true si las tramas son iguales; false si las tramas son distintas
	 */
	public boolean comprobarIntegridad(byte[] tramaCifrada){

		int crcLlega = ((int) tramaCifrada[20]) & 0xFF | (((int) tramaCifrada[21]) & 0xFF) << 8;
		int crcTrama = crc16(tramaCifrada);
		if(crcTrama == crcLlega){
			return true;
		}
		else{
			System.out.println("el contenido de la trama recibida no es igual al contenido "
					+ "de la trama enviada " + " trama enviada " + crcTrama + " crc " +  crcLlega);
			return false;
		}
